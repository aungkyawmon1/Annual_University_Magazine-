...
<div class="container">
    ...
    <div style="color: green">You can only comment only in 14 days.
    </div>
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active"
                    aria-current="page">{{ $magazine->user->first_name }} {{ $magazine->user->last_name }}</li>
            </ol>
        </nav>
    </div>
        ...
        <div class="col-12 col-md-4 col-lg-3 mb-4">
            <div class="post-container">
                ...
                <div class="d-flex justify-content-between mt-2 gap-3">
                    ...
                    $disabledClass = ($days > 14) ? 'disabled' : '';
                    ?>
                    <a href="{{ route('student.preview', ['magazine' => $magazine->magazine_id]) }}" target="_blank"
                       class="btn btn-outline w-50">Preview</a>


                    <div type="button" class="myDIV btn btn-primary w-50 <?php echo $disabledClass; ?>" data-bs-toggle="modal"
                         data-bs-target="#commentsModal" data-toggle="tooltip" data-placement="top">
            ...
            <div class="mt-2">
                <form action="{{ route('publishMagazine', ['id' => $magazine->magazine_id]) }}" method="POST">
                    ...
                    <button type="submit"
                            class="btn btn-primary w-100" {{ $magazine->is_published ? 'disabled' : '' }}>
                        Publish
                    </button>
                </form>
            </div>
    ...
    <div class="modal fade" id="commentsModal" tabindex="-1" aria-labelledby="commentsModalLabel"
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
            ...
            <form method="POST"
                  action="{{ route('coordinator.comment.post', ['magazine' => $magazine->magazine_id]) }}">
                @csrf
                <div class="modal-body">
                ...
                <div class="modal-body">
                            <textarea name="comment" class="form-control" rows="4" placeholder="Enter your comment here"
                                      required></textarea>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
