            ...
            <div class="post-container">
                ...
                <div class="mt-2">
                    <form action="{{ route('publishMagazine', ['id' => $magazine->magazine_id]) }}" method="POST">
                        <?php
                        // is published or not
                        $isPublished = $magazine->is_published;

                        ?>
                        @csrf <!-- CSRF token for security -->
{{--                        disable the button if the magazine is published--}}
                        <button type="submit" class="btn btn-primary w-100" {{ $isPublished ? 'disabled' : '' }}>Publish</button>
                    </form>
                </div>
            </div>
        </div>
